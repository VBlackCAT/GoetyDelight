...
public class SoulLichEntity extends Summoned {
    ...
    public void tick() {
        ...
        flying = target != null && target.isAlive();
        ...
        if (flying) {
            ...
            double motionY = (desiredY - this.getY()) * 0.15D;
            ...
            if (casting) {
                ...
                if (!spellFired && castingTicks <= 20 && target != null && target.isAlive()) {
                    spellFired = true;
    ...
    public static AttributeSupplier createAttributes() {
        return Summoned.createMobAttributes()
                ...
                .add(ModAttributes.COOLDOWN_DISCOUNT, 0.5D)
                .build();
    }
    ...
    private class SoulBoltGoal extends Goal {
        ...
        public boolean canUse() {
            LivingEntity target = SoulLichEntity.this.getTarget();
        ...
        public void tick() {
            ...
            SoulLichEntity.this.getLookControl().setLookAt(target, 30.0F, 30.0F);
            ...
            if (distance > ATTACK_DISTANCE) {
                Vec3 targetPos = target.position().add(0.0D, target.getBbHeight() + 2.5D, 0.0D);
                Vec3 direction = targetPos.subtract(SoulLichEntity.this.position());

                if (direction.lengthSqr() > 0.01D) {
                    direction = direction.normalize().scale(0.25D);
                    SoulLichEntity.this.setDeltaMovement(direction.x,
                            SoulLichEntity.this.getDeltaMovement().y, direction.z);

                    float raw = (float) (Mth.atan2(direction.z, direction.x) * (180F / Math.PI)) - 90.0F;
                ...
                SoulLichEntity.this.setDeltaMovement(0.0D,
                        SoulLichEntity.this.getDeltaMovement().y, 0.0D);
                ...
                if (spellCooldown <= 0) {
                    ...
                    setAttacking(true);
                    castingTicks = 45;
                    SoulLichEntity.this.lookAt(target, 50.0F, 50.0F);
                    spellCooldown = 50;
                }
            }
        }
