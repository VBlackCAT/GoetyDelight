...
public class SoulLichModel<T extends SoulLichEntity> extends HierarchicalModel<T> {
	...
	public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		if (transitionProgress < 1.0F && previousAnimationState != null && previousAnimationDefinition != null) {
			...
			applyTransitionPose(
					transitionProgress,
					previousAnimationState, previousAnimationDefinition,
					targetState, targetDefinition,
					ageInTicks
			);
		} else {
	...
	private void applyTransitionPose(
			float progress,
			AnimationState prevState, AnimationDefinition prevDef,
			AnimationState targetState, AnimationDefinition targetDef,
			float ageInTicks
	) {
