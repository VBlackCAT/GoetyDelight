...
public class GhostFarmerEntity extends AbstractWraith implements Merchant {
    ...
    protected void updateTrades() {
        ...
        this.offers.add(new MerchantOffer(
                new ItemCost(Items.MELON_SEEDS, 1),
                java.util.Optional.of(new ItemCost(ECTOPLASM.get(), 2)),
                new ItemStack(net.v_black_cat.goetydelight.init.ModItems.ECTOPLASMIC_MELON_SEEDS.get(), 1),
                12, 0, 0.0f
        ));

        this.offers.add(new MerchantOffer(
                new ItemCost(ECTOPLASM.get(), 18),
                java.util.Optional.of(new ItemCost(Items.MELON, 1)),
                new ItemStack(ModBlocks.ECTOPLASMIC_MELON_BLOCK.get().asItem(), 1),
                12, 0, 0.0f
        ));

        this.offers.add(new MerchantOffer(
                new ItemCost(ECTOPLASM.get(), 3),
                java.util.Optional.of(new ItemCost(Items.MELON_SLICE, 1)),
                new ItemStack(net.v_black_cat.goetydelight.init.ModItems.ECTOPLASMIC_MELON.get(), 1),
        ...
        for (BlockPos stemPos : plantStemsPos) {
            if (!attachedStems.contains(stemPos)) {
                ...
                if (block instanceof StemBlock) {
                    ...
                    if (age >= MAX_GROWTH_STAGE) {
                        for (int i = 0; i < 10; i++) {
                            this.level().getBlockState(stemPos).randomTick(
                                    (ServerLevel) this.level(), stemPos, this.level().random);
                        }
                    }
    ...
    @Nullable
    @Override
    public Entity changeDimension(net.minecraft.world.level.portal.DimensionTransition transition) {
        this.stopTrading();
