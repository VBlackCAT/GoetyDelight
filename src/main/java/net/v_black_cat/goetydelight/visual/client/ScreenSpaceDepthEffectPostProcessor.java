...
public final class ScreenSpaceDepthEffectPostProcessor {
    ...
    private static final ResourceLocation SHADER = ResourceLocation.fromNamespaceAndPath(GoetyDelight.MODID, "entity_depth_reconstruct");
    ...
    private static void runPostPass(Minecraft minecraft, RenderTarget mainTarget, TextureTarget outTarget, RenderLevelStageEvent event, EffectPacket packet) {
        ...
        shader.safeGetUniform("Time").set((event.getRenderTick() + event.getPartialTick().getGameTimeDeltaPartialTick(true)) / 20.0F);
    ...
    private static final class EffectPacket {
        ...
        private static float progress(Entity entity, ActiveEntityVisualEffect effect, RenderLevelStageEvent event) {
            if (effect.initialDuration() > 0) {
                return Mth.clamp(1.0F - (effect.remainingTicks() - event.getPartialTick().getGameTimeDeltaPartialTick(true)) / (float) effect.initialDuration(), 0.0F, 1.0F);
            }

            long start = effect.data().contains("StartGameTime") ? effect.data().getLong("StartGameTime") : entity.level().getGameTime();
            return Mth.clamp((entity.level().getGameTime() + event.getPartialTick().getGameTimeDeltaPartialTick(true) - start) / 80.0F, 0.0F, 1.0F);
        }
        ...
        private void add(Entity entity, ActiveEntityVisualEffect effect, RenderLevelStageEvent event, int mode) {
            Vec3 center = center(entity, event.getPartialTick().getGameTimeDeltaPartialTick(true), mode);
            ...
            float phase = (event.getRenderTick() + event.getPartialTick().getGameTimeDeltaPartialTick(true)) * 0.08F + count * 1.37F;
