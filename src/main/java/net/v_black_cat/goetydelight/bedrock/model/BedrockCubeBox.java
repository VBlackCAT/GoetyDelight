package net.v_black_cat.goetydelight.bedrock.model;
...
public class BedrockCubeBox implements BedrockCube {
    ...
    public void compile(PoseStack.Pose pose, Vector3f[] normals, VertexConsumer consumer, int lightmap, int overlay, float r, float g, float b, float a) {
        ...
        for (int i = 0; i < NUM_CUBE_FACES; i++) {
            // 1.21 使用 addVertex 替代 vertex，颜色参数使用 int ARGB 格式
            consumer.addVertex(
                    VERTICES[VERTEX_ORDER[i][0]].x, VERTICES[VERTEX_ORDER[i][0]].y, VERTICES[VERTEX_ORDER[i][0]].z,
                    color,
                    uvs[uvOrder[i][1]], uvs[uvOrder[i][2]],
                    overlay, lightmap,
                    normals[i].x, normals[i].y, normals[i].z
            );

            consumer.addVertex(
                    VERTICES[VERTEX_ORDER[i][1]].x, VERTICES[VERTEX_ORDER[i][1]].y, VERTICES[VERTEX_ORDER[i][1]].z,
                    color,
                    uvs[uvOrder[i][0]], uvs[uvOrder[i][2]],
                    overlay, lightmap,
                    normals[i].x, normals[i].y, normals[i].z
            );

            consumer.addVertex(
                    VERTICES[VERTEX_ORDER[i][2]].x, VERTICES[VERTEX_ORDER[i][2]].y, VERTICES[VERTEX_ORDER[i][2]].z,
                    color,
                    uvs[uvOrder[i][0]], uvs[uvOrder[i][3]],
                    overlay, lightmap,
                    normals[i].x, normals[i].y, normals[i].z
            );

            consumer.addVertex(
                    VERTICES[VERTEX_ORDER[i][3]].x, VERTICES[VERTEX_ORDER[i][3]].y, VERTICES[VERTEX_ORDER[i][3]].z,
                    color,
                    uvs[uvOrder[i][1]], uvs[uvOrder[i][3]],
                    overlay, lightmap,
                    normals[i].x, normals[i].y, normals[i].z
            );
        }
