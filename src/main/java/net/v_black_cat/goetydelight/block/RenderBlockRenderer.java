...
public class RenderBlockRenderer implements BlockEntityRenderer<RenderBlockEntity> {
    ...
    private static final ResourceLocation DUMMY_TEXTURE =
            ResourceLocation.fromNamespaceAndPath(GoetyDelight.MODID, "textures/effect/test_texture.png");
    ...
    private void renderQuad(PoseStack poseStack, VertexConsumer buffer, Vec3[] vertices,
        ...
        buffer.addVertex(pose, (float) vertices[0].x, (float) vertices[0].y, (float) vertices[0].z)
                .setUv(0, 0)
                .setOverlay(packedOverlay)
                .setLight(packedLight);

        buffer.addVertex(pose, (float) vertices[1].x, (float) vertices[1].y, (float) vertices[1].z)
                .setUv(0, 1)
                .setOverlay(packedOverlay)
                .setLight(packedLight);

        buffer.addVertex(pose, (float) vertices[2].x, (float) vertices[2].y, (float) vertices[2].z)
                .setUv(1, 1)
                .setOverlay(packedOverlay)
                .setLight(packedLight);

        buffer.addVertex(pose, (float) vertices[3].x, (float) vertices[3].y, (float) vertices[3].z)
                .setUv(1, 0)
                .setOverlay(packedOverlay)
                .setLight(packedLight);
