...
/** 统一管理所有附件类型的注册。 附件用于为实体等对象附加临时或持久化数据。 */
public class ModAttachments {

    private static final StreamCodec<
            ByteBuf,
            ActiveBuffs> ACTIVE_BUFFS_STREAM_CODEC = BuffInstance.STREAM_CODEC.apply(ByteBufCodecs.list())
            .map(ModAttachments::toActiveBuffs, ModAttachments::toInstanceList);

    private static final StreamCodec<
            ByteBuf,
            EntityVisualEffects> VISUAL_EFFECTS_STREAM_CODEC = ByteBufCodecs.COMPOUND_TAG.map(
            tag -> {
    ...
    public static final DeferredRegister<
            AttachmentType<
                    ?>> ATTACHMENT_TYPES = DeferredRegister.create(NeoForgeRegistries.ATTACHMENT_TYPES, GoetyDelight.MODID);

    public static final Supplier<
            AttachmentType<ActiveBuffs>> ACTIVE_BUFFS = ATTACHMENT_TYPES.register("active_buffs",
    ...
    public static final Supplier<
            AttachmentType<
                    EntityVisualEffects>> VISUAL_EFFECTS = ATTACHMENT_TYPES.register("visual_effects",
    ...
    public static final Supplier<
            AttachmentType<
                    Integer>> MINION_STEW_BOOST_COUNT = registerIntAttachment(MinionBoostType.STEW);
    public static final Supplier<
            AttachmentType<
                    Integer>> MINION_SOUP_BOOST_COUNT = registerIntAttachment(MinionBoostType.SOUP);
