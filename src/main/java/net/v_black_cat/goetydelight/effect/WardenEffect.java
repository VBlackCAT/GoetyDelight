...
public class WardenEffect extends MobEffect {
    ...
    // 缓存 WardenEffect 的 Holder，避免重复查找
    private static Holder<MobEffect> cachedHolder = null;
    ...
    /**
     * 获取 WardenEffect 的 Holder（带缓存）
     */
    private static Holder<MobEffect> getWardenHolder() {
        if (cachedHolder == null) {
            // 使用 Optional.map 转换类型
            Optional<Holder<MobEffect>> optional = BuiltInRegistries.MOB_EFFECT
                    .getHolder(ModEffects.WARDEN.getId())
                    .map(holder -> (Holder<MobEffect>) holder);
            if (optional.isPresent()) {
                cachedHolder = optional.get();
            } else {
                throw new IllegalStateException("WardenEffect not registered!");
            }
        }
        return cachedHolder;
    }
    ...
    /**
     * 获取实体身上的 WardenEffect 实例
     */
    @Nullable
    private static WardenEffect getWardenEffectInstance(LivingEntity entity) {
        if (entity == null) return null;
        Holder<MobEffect> holder = getWardenHolder();
        MobEffectInstance instance = entity.getEffect(holder);
        if (instance != null && instance.getEffect().value() instanceof WardenEffect effect) {
            return effect;
        }
        return null;
    }
    ...
    public static boolean hasDamageBoostAgainst(LivingEntity attacker, LivingEntity target) {
        ...
        WardenEffect effect = getWardenEffectInstance(attacker);
        if (effect != null) {
            // 检查 Buff
    ...
    private static class EffectVibrationUser implements VibrationSystem.User {
        ...
        public void onReceiveVibration(ServerLevel level, BlockPos pos, Holder<GameEvent> event,
            ...
            if (source instanceof LivingEntity target) {
                WardenEffect effect = getWardenEffectInstance(owner);
                if (effect != null) {
