...
public final class OrbitingSphereRenderer {
    ...
    private static void drawSphere(PoseStack.Pose pose, float renderTime, int alpha) {
        ...
        BufferBuilder buffer = Tesselator.getInstance().begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR_NORMAL);
        ...
        for (int stack = 0; stack < STACKS; stack++) {
            ...
            for (int slice = 0; slice < SLICES; slice++) {
                ...
                addSphereVertex(buffer, matrix, pose, theta0, phi0, renderTime, alpha);
                addSphereVertex(buffer, matrix, pose, theta1, phi0, renderTime, alpha);
                addSphereVertex(buffer, matrix, pose, theta1, phi1, renderTime, alpha);
                addSphereVertex(buffer, matrix, pose, theta0, phi1, renderTime, alpha);
            }
        }
    ...
    private static void addSphereVertex(BufferBuilder buffer, Matrix4f matrix, PoseStack.Pose pose, float theta, float phi, float renderTime, int alpha) {
